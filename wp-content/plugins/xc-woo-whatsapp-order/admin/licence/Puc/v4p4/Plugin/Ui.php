...
if ( !class_exists('Puc_v4p4_Plugin_Ui', false) ):
	...
	class Puc_v4p4_Plugin_Ui {
		...
		public function addCheckForUpdatesLink($pluginMeta, $pluginFile) {
			if ( $this->isMyPluginFile($pluginFile) ) {
				$linkUrl = wp_nonce_url(
					add_query_arg(
						array(
							'ec_woo_check_for_updates' => 1,
							'ec_woo_slug'              => $this->updateChecker->slug,
						),
						self_admin_url('plugins.php')
					),
					'ec_woo_check_for_updates'
				);
		...
		public function handleManualCheck() {
			$shouldCheck =
				isset($_GET['ec_woo_check_for_updates'], $_GET['ec_woo_slug'])
				&& $_GET['ec_woo_slug'] == $this->updateChecker->slug
				&& check_admin_referer('ec_woo_check_for_updates');
			...
			if ( $shouldCheck ) {
				...
				wp_redirect(add_query_arg(
					array(
						'ec_woo_update_check_result' => $status,
						'ec_woo_slug'                => $this->updateChecker->slug,
					),
					self_admin_url('plugins.php')
				));
		...
		public function displayManualCheckResult() {
			if ( isset($_GET['ec_woo_update_check_result'], $_GET['ec_woo_slug']) && ($_GET['ec_woo_slug'] == $this->updateChecker->slug) ) {
				$status = strval($_GET['ec_woo_update_check_result']);
